...
namespace aspect
{
  namespace internal
  {
                                       ...
                                       const dealii::LinearAlgebra::distributed::BlockVector<double>  &src) const
    {
      ...
      {
        double r = dst.block(1).l2_norm();
        std::cout << "before pressure mean " << r << std::endl;
        dst.block(1).add(r/dst.block(1).size());
        r = dst.block(1).l2_norm();
        std::cout << "after pressure mean " << r << std::endl;
    ...
    if (print_details)
      {
        ...
        const double imbalance = 0.;//MGTools::workload_imbalance(sim.triangulation);
...
//                                                            *sim.mapping);
  ...
  {
    AssertThrow(false, ExcNotImplemented("get_dof_handler_projection not implemented"));
