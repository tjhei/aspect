...
namespace aspect
{
  namespace BoundaryVelocity
  {
    ...
    template <int dim>
    Tensor<1,dim>
    Interface<dim>::boundary_velocity (const types::boundary_id /*boundary_indicator*/,
                                       const Point<dim> &/*position*/) const
    {
      Assert (false, ExcMessage ("A derived class needs to override the boundary_velocity function."));
      return Tensor<1,dim>();
    }
